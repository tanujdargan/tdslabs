...
// Every form is tiny except the HTML import, whose body can be several MB, so
// that one route gets a large limit while all other POSTs stay tightly capped.
const smallForm = express.urlencoded({ extended: false, limit: '256kb' });
const largeForm = express.urlencoded({ extended: false, limit: '64mb' });
app.use((req, res, next) => {
  if (req.method === 'POST' && req.path === '/import') return largeForm(req, res, next);
  return smallForm(req, res, next);
});
